...
func (c *CodecRequest) Method() (string, error) {
	//Modifications here to handle weird way SDK is call RPC methods
	m := c.request.Method
	if len(m) > 1 && c.err == nil {
		parts := strings.Split(m, ".")
		if len(parts) == 2 {
			return m, nil
		} else {
			return methodMap[m], nil
		}
	}
	return "", c.err
}
...
func (c *CodecRequest) writeServerResponse(w http.ResponseWriter, res *serverResponse) {
	...
	/*DUE TO SDK NOT FOLLOWING jsonrpc 2.0 protocol, we must always set an id since it sends a null one which
	would interpret it as a notification*/
	if c.request.Id == nil {
