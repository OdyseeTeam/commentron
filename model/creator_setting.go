...
type CreatorSetting struct {
	...
	CreatorSetting null.JSON `boil:"creator_setting" json:"creator_setting,omitempty" toml:"creator_setting" yaml:"creator_setting,omitempty"`
...
var CreatorSettingTableColumns = struct {
	...
	CommentsEnabled:            "creator_setting.comments_enabled",
	MinTipAmountComment:        "creator_setting.min_tip_amount_comment",
	MinTipAmountSuperChat:      "creator_setting.min_tip_amount_super_chat",
	MutedWords:                 "creator_setting.muted_words",
	CreatedAt:                  "creator_setting.created_at",
	UpdatedAt:                  "creator_setting.updated_at",
	SlowModeMinGap:             "creator_setting.slow_mode_min_gap",
	CurseJarAmount:             "creator_setting.curse_jar_amount",
	IsFiltersEnabled:           "creator_setting.is_filters_enabled",
	ChatOverlay:                "creator_setting.chat_overlay",
	ChatOverlayPosition:        "creator_setting.chat_overlay_position",
	ChatRemoveComment:          "creator_setting.chat_remove_comment",
	StickerOverlay:             "creator_setting.sticker_overlay",
	StickerOverlayKeep:         "creator_setting.sticker_overlay_keep",
	StickerOverlayRemove:       "creator_setting.sticker_overlay_remove",
	ViewercountOverlay:         "creator_setting.viewercount_overlay",
	ViewercountOverlayPosition: "creator_setting.viewercount_overlay_position",
	ViewercountChatBot:         "creator_setting.viewercount_chat_bot",
	TipgoalOverlay:             "creator_setting.tipgoal_overlay",
	TipgoalAmount:              "creator_setting.tipgoal_amount",
	TipgoalOverlayPosition:     "creator_setting.tipgoal_overlay_position",
	TipgoalPreviousDonations:   "creator_setting.tipgoal_previous_donations",
	TipgoalCurrency:            "creator_setting.tipgoal_currency",
	TimeSinceFirstComment:      "creator_setting.time_since_first_comment",
	...
	CreatorSetting:             "creator_setting.creator_setting",
}
...
var CreatorSettingWhere = struct {
	...
	CreatorSetting             whereHelpernull_JSON
}{
	...
	CreatorSetting:             whereHelpernull_JSON{field: "`creator_setting`.`creator_setting`"},
}
...
func (r *creatorSettingR) GetCreatorChannel() *Channel {
	if r == nil {
		return nil
	}
...
type (
	...
	// This should almost always be used instead of []CreatorSetting.
...
func (creatorSettingL) LoadCreatorChannel(e boil.Executor, singular bool, maybeCreatorSetting interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeCreatorSetting.(*CreatorSetting)
		if !ok {
			object = new(CreatorSetting)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeCreatorSetting)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeCreatorSetting))
			}
		}
	} else {
		s, ok := maybeCreatorSetting.(*[]*CreatorSetting)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeCreatorSetting)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeCreatorSetting))
			}
		}
	}
...
func CreatorSettings(mods ...qm.QueryMod) creatorSettingQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`creator_setting`.*"})
	}

	return creatorSettingQuery{q}
}
